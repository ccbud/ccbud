...
(function () {
  var DICT = {
    en: {
      ...
      'f5.title': 'A privacy filter, on by your choice',
      'f5.desc': 'Optionally route outbound requests through on-device PII detection (Microsoft Presidio) that redacts names, keys, emails and more before anything leaves.',
      'f5.note': '<b>Fail-closed:</b> if the filter is enabled but not ready, requests are blocked rather than leaked. Privacy you can actually rely on.',
    ...
    zh: {
      ...
      'f5.title': '隐私过滤，开不开你说了算',
      'f5.desc': '可选地让出站请求先经过本机 PII 识别（Microsoft Presidio），在任何东西离开前抹去姓名、密钥、邮箱等敏感信息。',
      'f5.note': '<b>失败即拦截：</b>若过滤已开启但尚未就绪，请求会被拦下而不是泄露出去。这才是靠得住的隐私。',
    ...
    'zh-TW': {
      ...
      'f5.title': '隱私過濾，開不開你決定',
      'f5.desc': '可選擇讓出站請求先經過本機 PII 偵測（Microsoft Presidio），在任何東西離開前抹去姓名、金鑰、Email 等敏感資訊。',
      'f5.note': '<b>失敗即攔截：</b>若過濾已啟用但尚未就緒，請求會被攔下而非外洩。這才是靠得住的隱私。',
    ...
    ja: {
      ...
      'f5.title': 'プライバシーフィルター、有効化はあなた次第',
      'f5.desc': '送信リクエストを端末内の PII 検出（Microsoft Presidio）に通し、名前・鍵・メールなどを外へ出る前に伏せることもできます。',
      'f5.note': '<b>フェイルクローズ：</b>フィルターが有効でも未準備なら、リクエストは漏らさず遮断します。本当に頼れるプライバシー。',
    ...
    ko: {
      ...
      'f5.title': '프라이버시 필터, 켜고 끄기는 당신 선택',
      'f5.desc': '선택적으로 송신 요청을 기기 내 PII 탐지(Microsoft Presidio)에 통과시켜, 이름·키·이메일 등을 나가기 전에 가립니다.',
      'f5.note': '<b>실패 시 차단:</b> 필터가 켜졌지만 준비되지 않았다면, 요청은 새는 대신 차단됩니다. 정말로 믿을 수 있는 프라이버시.',
